	...
	return {
		...
		alternates: {
			languages: {
				...
				ja: localizedPath('ja', item.slug),
				zh: localizedPath('zh', item.slug),
				es: localizedPath('es', item.slug),
				pt: localizedPath('pt', item.slug),
				fr: localizedPath('fr', item.slug),
				de: localizedPath('de', item.slug),
				ar: localizedPath('ar', item.slug),
			},
		},
	}
}
		...
		<main className="min-h-screen bg-black pt-28 px-6">
			<article className="max-w-3xl mx-auto">
				...
				<div className="space-y-4 text-white/80 leading-7">
					<p>
						หน้านี้เป็นตัวอย่างคอนเทนต์หลายภาษาพร้อม <code className="mx-1">hreflang</code> และ ISR โดยใช้โครงสร้าง URL แบบ
						<code className="mx-1">/{`{locale}`}/{`{slug}`}/</code>
					</p>
					<p>
