...
export function PageViewTracker() {
	...
	const getPageCategory = (path: string) => {
		...
		if (segments.length >= 1) {
			// main
			if (segments.length === 2 && segments[1] === 'main') return 'main'
			// category group under /[locale]/c/[category]
			if (segments.length >= 3 && segments[1] === 'c') return `category:${segments[2]}`
			// subcategory content: /[locale]/[slug]
			if (segments.length === 2) return `subcategory:${segments[1]}`
		}
